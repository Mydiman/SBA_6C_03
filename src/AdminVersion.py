...
from .According import filmname_accord, dimension_accord, language_accord, housename_accord, ticket_accord
...
def print_402(filter_film, filter_dimension, filter_language, filter_house, filter_after, filter_before) -> int:
    print(Colour.Underline + "Showing filtering" + Colour.Reset)
    print("")
    print("Filter showings with film         : " + filter_film)
    print("Filter showings with dimension    : " + filter_dimension)
    print("Filter showings with language     : " + filter_language)
    print("Filter showings with house        : " + filter_house)
    if filter_after != "" and filter_before == "":
        print("Filter showings play after        : " + filter_after)
    elif filter_after == "" and filter_before != "":
        print("Filter showings play before       : " + filter_before)
    elif filter_after == "" and filter_before == "":
        print("Filter showings play within period: (Not yet inputed) - (Not yet inputed)")
    else:
        print("Filter showings play within       : " + filter_after + " - " + filter_before)
    print("")
    print("1 <-- Change film")
    print("2 <-- Change dimension")
    print("3 <-- Change language")
    print("4 <-- Change house")
    print("5 <-- Change period")
    print("6 <-- Confirm")
    print("7 <-- Go back")
    print("")
    return 7
...
def print_403() -> int:
    print(Colour.Underline + "Filtering with film" + Colour.Reset)
    file_path = path.join(commond_path, 'Film', 'filmname.txt')
    f = open(file_path, "r")
    count_film = int(f.readline() [:-1])
    len_count_film = len(str(count_film + 2))
    for i in range(1, count_film + 1):
        print("{:<{}} <-- {}".format(i, len_count_film, f.readline() [:-1]))
    print("{:<{}} <-- Stop filtering with film".format(i + 1, len_count_film))
...
def print_404(filter_dimension, filter_film) -> int:
    print(Colour.Underline + "Filtering with dimension" + Colour.Reset)
    if filter_film == "":
        file_path = path.join(commond_path, 'Film', 'filmname.txt')
        f = open(file_path, "r")
        count_film = int(f.readline() [:-1])
        ...
        for i in range(count_film):
            ...
            g = open(file_path, "r")
            count_dimension_film = int(g.readline() [:-1])
            for j in range(count_dimension_film):
                line = g.readline() [:-2]
                if not line in dimension:
                    count_dimension += 1
                    dimension.append(int(line))
            g.close()
        len_count_dimension = len(str(count_dimension + 2))
        for i in range(count_dimension):
            for j in range(i, count_dimension - 1):
                if dimension [j] > dimension [j + 1]:
                    temp = dimension [j]
                    dimension [j] = dimension [j + 1]
                    dimension [j + 1] = temp
        for i in range(1, count_dimension + 1):
            print("{:<{}} <-- {}D".format(i, len_count_dimension, str(dimension [i - 1])))
        print("{:<{}} <-- Stop filtering with dimension".format(i + 1, len_count_dimension))
        print("{:<{}} <-- Go back".format(i + 2, len_count_dimension))
        print("")
        return i + 2
    else:
        file_path = path.join(commond_path, 'Film', filter_film, 'dimension.txt')
        f = open(file_path, "r")
        count_dimension = int(f.readline() [:-1])
        len_count_dimension = len(str(count_dimension + 2))
        for i in range(1, count_dimension + 1):
            print("{:<{}} <-- {}".format(i, len_count_dimension, f.readline() [:-1]))
        print("{:<{}} <-- Stop filtering with dimension".format(i + 1, len_count_dimension))
        print("{:<{}} <-- Go back".format(i + 2, len_count_dimension))
        print("")
        return i + 2
...
def print_405(filter_language, filter_film) -> int:
    print(Colour.Underline + "Filtering with language" + Colour.Reset)
    if filter_film == "":
        file_path = path.join(commond_path, 'Film', 'filmname.txt')
        f = open(file_path, "r")
        count_film = int(f.readline() [:-1])
        ...
        for i in range(count_film):
            ...
            g = open(file_path, "r")
            count_language_film = int(g.readline() [:-1])
            for j in range(count_language_film):
                line = g.readline() [:-1]
                if not line in language:
                    count_language += 1
                    language.append(line)
            g.close()
        len_count_language = len(str(count_language + 2))
        language.sort(key = lambda v: v.lower())
        for i in range(1, count_language + 1):
            print("{:<{}} <-- {}".format(i, len_count_language, language [i - 1]))
        print("{:<{}} <-- Stop filtering with language".format(i + 1, len_count_language))
        print("{:<{}} <-- Go back".format(i + 2, len_count_language))
        return i + 2
    else:
        file_path = path.join(commond_path, 'Film', filter_film, 'language.txt')
        f = open(file_path, "r")
        count_language = int(f.readline() [:-1])
        len_count_language = len(str(count_language + 2))
        for i in range(1, count_language + 1):
            print("{:<{}} <-- {}".format(i, len_count_language, f.readline() [:-1]))
        print("{:<{}} <-- Stop filtering with language".format(i + 1, len_count_language))
        print("{:<{}} <-- Go back".format(i + 2, len_count_language))
        print("")
        return i + 2
...
def print_406() -> int:
    print(Colour.Underline + "Filtering with house" + Colour.Reset)
    file_path = path.join(commond_path, 'House', 'housename.txt')
    f = open(file_path, "r")
    count_house = int(f.readline() [:-1])
    len_count_house = len(str(count_house + 2))
    for i in range(1, count_house + 1):
        print("{:<{}} <-- {}".format(i, len_count_house, f.readline() [:-1]))
    print("{:<{}} <-- Stop filtering with film".format(i + 1, len_count_house))
...
def print_407(filter_after, filter_before) -> int:
    print(Colour.Underline + "Filtering showings play within a period" + Colour.Reset)
    print("")
    if filter_after != "" and filter_before == "":
        print("Filter showings play within period: " + filter_after + " - (Not yet inputed)")
        print("Period starting time              : " + filter_after)
        print("Period ending time                : (Not yet inputed)")
    elif filter_after == "" and filter_before != "":
        print("Filter showings play within period: (Not yet inputed) - " + filter_before)
        print("Period starting time              : (Not yet inputed)")
        ...
        print("Filter showings play within period: " + filter_after + " - " + filter_before)
        print("Period starting time              : " + filter_after)
        print("Period ending time                : " + filter_before)
    print("")
    print("1 <-- Change the period starting time")
    print("2 <-- Change the period ending time")
    print("3 <-- Stop filtering with period")
    print("4 <-- Go back")
    print("")
    return 4
...
def print_408_409(filter_after, filter_before) -> None:
...
def print_410(filter_film, filter_dimension, filter_language, filter_house, filter_after, filter_before):
    showing_list = []
    real_showing_list = []
    count_real_showing_list = 0
    if filter_film != "" and filter_dimension != "" and filter_language != "":
        file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, filter_dimension, filter_language, 'Showingnum.txt')
        f = open(file_path, "r")
        count_showing_film = int(f.readline())
        f.close()
        file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, filter_dimension, filter_language, 'Showing.txt')
        f = open(file_path, "r")
        for j in range(count_showing_film):
            f.readline()
            showing_list.append(f.readline() [:-1])
        f.close()
    elif filter_film != "" and filter_dimension != "" and filter_language == "":
        file_path = path.join(commond_path, 'Film', filter_film, 'language.txt')
        f = open(file_path, "r")
        language = []
        count_language = int(f.readline() [:-1])
        ...
        f.close()
        for i in language:
            file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, filter_dimension, i, 'Showingnum.txt')
            f = open(file_path, "r")
            count_showing_film = int(f.readline())
            f.close()
            file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, filter_dimension, i, 'Showing.txt')
            f = open(file_path, "r")
            for j in range(count_showing_film):
                f.readline()
                showing_list.append(f.readline() [:-1])
        ...
        f.close()
        for i in dimension:
            file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, i, filter_language, 'Showingnum.txt')
            f = open(file_path, "r")
            count_showing_film = int(f.readline())
            f.close()
            file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, i, filter_language, 'Showing.txt')
            f = open(file_path, "r")
            for j in range(count_showing_film):
                f.readline()
                showing_list.append(f.readline() [:-1])
        ...
        count_film = int(f.readline() [:-1])
        for i in range(count_film):
            film.append(f.readline() [:-1])
        f.close()
        filter_film = []
        for i in film:
            dimension = 0
            language = 0
            file_path = path.join(commond_path, 'Film', i, 'dimension.txt')
            f = open(file_path, "r")
            count_dimension = int(f.readline() [:-1])
            for j in range(count_dimension):
                line = f.readline() [:-1]
                if line == filter_dimension:
                    dimension = 1
            f.close()
            file_path = path.join(commond_path, 'Film', i, 'language.txt')
            f = open(file_path, "r")
            count_language = int(f.readline() [:-1])
            for j in range(count_language):
                line = f.readline() [:-1]
                if line == filter_language:
                    language = 1
            f.close()
            if dimension == 1 and language == 1:
                filter_film.append(i)
        for i in filter_film:
            file_path = path.join(commond_path, 'Showing', '__Film information', i, filter_dimension, filter_language, 'Showingnum.txt')
            f = open(file_path, "r")
            count_showing_film = int(f.readline())
            f.close()
            file_path = path.join(commond_path, 'Showing', '__Film information', i, filter_dimension, filter_language, 'Showing.txt')
            f = open(file_path, "r")
            for j in range(count_showing_film):
                f.readline()
                showing_list.append(f.readline() [:-1])
        ...
        for i in dimension:
            for j in language:
                file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, i, j, 'Showingnum.txt')
                f = open(file_path, "r")
                count_showing_film = int(f.readline())
                f.close()
                file_path = path.join(commond_path, 'Showing', '__Film information', filter_film, i, j, 'Showing.txt')
                f = open(file_path, "r")
                for k in range(count_showing_film):
                    f.readline()
                    showing_list.append(f.readline() [:-1])
                f.close()
    elif filter_film == "" and filter_dimension != "" and filter_language == "":
        ...
        for i in film:
            ...
            for j in range(count_dimension):
                line = f.readline() [:-1]
                if line == filter_dimension:
                    count_dimension_film += 1
                    dimension_film.append(i)
            f.close()
        language_film = []
        for i in dimension_film:
            language = []
            file_path = path.join(commond_path, 'Film', i, 'language.txt')
            f = open(file_path, "r")
            count_language = int(f.readline() [:-1])
        ...
        for i in range(count_dimension_film):
            for j in language_film [i]:
                ...
                count_showing_film = int(f.readline())
                f.close()
                file_path = path.join(commond_path, 'Showing', '__Film information', dimension_film [i], filter_dimension, j, 'Showing.txt')
                f = open(file_path, "r")
                for k in range(count_showing_film):
                    f.readline()
                    showing_list.append(f.readline() [:-1])
        ...
        for i in range(count_dimension_film):
            for j in language_film [i]:
                ...
                count_showing_film = int(f.readline())
                f.close()
                file_path = path.join(commond_path, 'Showing', '__Film information', dimension_film [i], j, filter_language, 'Showing.txt')
                f = open(file_path, "r")
                for k in range(count_showing_film):
                    f.readline()
                    showing_list.append(f.readline() [:-1])
        ...
        for i in range(count_film):
            line = f.readline() [:-1]
            film.append(line)
            g = open(path.join(commond_path, 'Film', line, 'dimension.txt'))
            count_dimension = int(g.readline() [:-1])
            temp = []
            for j in range(count_dimension):
                temp.append(g.readline() [:-1])
            dimension.append(temp)
            g.close()
            g = open(path.join(commond_path, 'Film', line, 'language.txt'))
            ...
            g.close()
        for i in range(count_film):
            for j in dimension [i]:
                for k in language [i]:
                    file_path = path.join(commond_path, 'Showing', '__Film information', film [i], j, k, 'Showingnum.txt')
                    f = open(file_path)
                    count_showing = int(f.readline())
                    file_path = path.join(commond_path, 'Showing', '__Film information', film [i], j, k, 'Showing.txt')
                    f = open(file_path, "r")
                    for l in range(count_showing):
                        f.readline()
                        showing_list.append(f.readline() [:-1])
    format = "%d/%m/%y %H:%M:%S"
    if filter_house != "" and filter_after != "" and filter_before != "":
        datetime_after = datetime.strptime(filter_after, format)
        datetime_before = datetime.strptime(filter_before, format)
        file_path = path.join(commond_path, 'Showing', '__House information', filter_house, 'Showingnum.txt')
        f = open(file_path, "r")
        count_house_time = int(f.readline())
        f.close()
        file_path = path.join(commond_path, 'Showing', '__House information', filter_house, 'Time.txt')
        f = open(file_path, "r")
        for i in range(count_house_time):
            line1 = datetime.strptime(f.readline() [:-1], format)
            line2 = f.readline() [:-1]
            line3 = f.readline() [:-1]
            if line3 in showing_list:
                if line1 >= datetime_after and line1 <= datetime_before:
                    real_showing_list.append(line3)
                    count_real_showing_list += 1
    elif filter_house != "" and filter_after != "" and filter_before == "":
        datetime_after = datetime.strptime(filter_after, format)
        file_path = path.join(commond_path, 'Showing', '__House information', filter_house, 'Showingnum.txt')
        f = open(file_path, "r")
        count_house_time = int(f.readline())
        ...
        for i in range(count_time):
            ...
            if line3 in showing_list:
                real_showing_list.append(line3)
                count_real_showing_list += 1
    print(Colour.Underline + "Showing list" + Colour.Reset)
    print("")
    if count_real_showing_list == 0:
        print("There is no showing matching condition.")
        print("1 <-- Go back")
        print("")
    else:
        showing = real_showing_list
        count_showing = count_real_showing_list
        ...
        for i in range(count_showing):
            ...
            f = open(file_path, "r")
            starttime = datetime.strptime(f.readline(), format)
            f.close()
            file_path = path.join(commond_path, 'Film', filmname, 'timelength.txt')
            f = open(file_path, "r")
            endtime = starttime + timedelta(minutes = int(f.readline()))
            f.close()
            print("{:<{}}     Time          : {} - {}".format(" ", len_count_showing, starttime.strftime(format), endtime.strftime(format)))
        print("{} <-- Go back".format(i + 2))
        print("")
    return_vl  = []
    return_vl.append(count_real_showing_list + 1)
    return_vl.append(real_showing_list)
    return return_vl
    ...
    while stage != 0:
        ...
        #house > seatingplan > rename
            ...
            if str_input == "":
                ...
                input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
            elif str_input [len(str_input) - 1] == ".":
                str_input = ""
                print(Colour.Red + "NAME OF A HOUSE CANNOT END WITH \".\"" + Colour.Reset)
                input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                ...
                input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
            elif len(str_input) >= 250:
                str_input = ""
                print(Colour.Red + "HOUSE NAME CANNOT BE TOO LONG" + Colour.Reset)
                input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
            file_path = path.join(commond_path, "House", "housename.txt")
            f = open(file_path, "r")
            count_house = int(f.readline() [:-1])
            for i in range(count_house):
                if "HOUSE " + str_input.upper() == f.readline() [:-1].upper() and "HOUSE " + str_input.upper() != house.upper():
                    str_input = ""
        ...
        #house > creation
            ...
            while name == "":
                ...
                count_house = int(f.readline() [:-1])
                for i in range(count_house):
                    if "HOUSE " + name.upper() == f.readline() [:-1].upper():
                        name = ""
                        print(Colour.Red + "NAME OF A HOUSE CANNOT BE THE SAME AS OTHER HOUSES" + Colour.Reset)
                        input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
            print("")
        ...
        #film > data > name
            ...
            while str_input == "":
                ...
                if str_input == "":
                    ...
                    str_input = ""
                    print(Colour.Red + "FILM NAME CANNOT END WITH \".\"" + Colour.Reset)
                    input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                    ...
                    str_input = ""
                    print(Colour.Red + "FILM NAME CANNOT BE TOO LONG" + Colour.Reset)
                    input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                for i in range(count_film):
        ...
        #film > data > language > addition
            ...
            while str_input == "":
                ...
                if str_input == "":
                    ...
                    str_input = ""
                    print(Colour.Red + "NAME OF A LANGUAGE CANNOT END WITH \".\"" + Colour.Reset)
                    input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                elif file_name_include(str_input) == False:
                    str_input = ""
                    ...
                    str_input = ""
                    print(Colour.Red + "NAME OF A LANGUAGE CANNOT BE TOO LONG" + Colour.Reset)
                    input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                for i in range(count_language):
                    if str_input != 0:
        ...
        #film > data > dimension > addition
            ...
            while str_input == "":
                ...
                if str_input == "":
                    ...
                    str_input = ""
                    print(Colour.Red + "NAME OF A DIMENSION CANNOT BE TOO LONG" + Colour.Reset)
                    input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
                else:
                    str_input = ""
            ...
            for i in range(count_price):
                ...
                while str_input == "":
                    ...
                    if str_input == "":
                        ...
                        record1.append(str(int(str_input)))
            price = ""
            for i in range(count_price):
        ...
        #showing > filter
            ...
            choose = 0
            while int_input == 0 or int_input > choose:
                choose = print_402(filter_film, filter_dimension, filter_language, filter_house, filter_after, filter_before)
                int_input = input_int_check("Input number: ", choose)
                clearscreen()
            if int_input == choose:
                stage = 401
        ...
        #showing > filter > film
            ...
            if int_input == choose - 1:
                ...
                for i in range(int_input):
                    f.readline()
                filter_film = f.readline() [:-1]
                f.close()
            stage = 402
        #showing > filter > dimension
        elif stage == 404:
            int_input = 0
            choose = 0
            while int_input == 0 or int_input > choose:
                choose = print_404(filter_dimension, filter_film)
                int_input = input_int_check("Input number: ", choose)
                clearscreen()
            if int_input == choose - 1:
                filter_dimension = ""
                    ...
                    for i in range(count_dimension):
                        for j in range(i, count_dimension - 1):
                            if dimension [j] > dimension [j + 1]:
                                ...
                                dimension [j] = dimension [j + 1]
                                dimension [j + 1] = temp
                    filter_dimension = str(dimension [int_input - 1]) + "D"
                else:
                    file_path = path.join(commond_path, 'Film', filter_film, 'dimension.txt')
                    f = open(file_path, "r")
                    for i in range(int_input):
                        f.readline()
                    filter_dimension = f.readline() [:-1]
            stage = 402
        #showing > filter > language
        elif stage == 405:
            int_input = 0
            choose = 0
            while int_input == 0 or int_input > choose:
                choose = print_405(filter_language, filter_film)
                int_input = input_int_check("Input number: ", choose)
                clearscreen()
            if int_input == choose - 1:
                filter_language = ""
                ...
                if filter_film == "":
                    ...
                    for i in range(count_film):
                        ...
                        g.close()
                    language.sort(key = lambda v: v.lower())
                    filter_language = str(language [int_input - 1]) 
                else:
                    file_path = path.join(commond_path, 'Film', filter_film, 'language.txt')
                    f = open(file_path, "r")
                    for i in range(int_input):
                        f.readline()
                    filter_language = f.readline() [:-1]
            stage = 402
        #showing > filter > house
        elif stage == 406:
            int_input = 0
            choose = 0
            while int_input == 0 or int_input > choose:
        ...
        #showing > filter > time
            ...
            choose = 0
            while int_input == 0 or int_input > choose:
                choose = print_407(filter_after, filter_before)
                int_input = input_int_check("Input number: ", choose)
                clearscreen()
            if int_input == choose - 1:
                filter_after = ""
                ...
                stage = 409
        #showing > filter > time > starting
        elif stage == 408:
            str_input = ""
            exit_input = 0
            while str_input == "" and exit_input == 0:
                clearscreen()
                print_408_409(filter_after, filter_before)
                str_input = input("Input new period starting time (follow the format): ")
                while str_input != "" and str_input [0] == " ":
                    temp = ""
                    for i in range(1, len(str_input)):
                        temp += str_input [i]
                ...
                if str_input == "exit" or str_input == "Exit" or str_input == "EXIT":
                    ...
                    if str_input == "":
                        ...
                        if len(str_input) == 17:
                            ...
                            input(Colour.Yellow + "ENTER TO RETRY" + Colour.Reset)
            if exit_input == 0:
                filter_after = str_input
            stage = 407
        #showing > filter > time > ending
        elif stage == 409:
            str_input = ""
            exit_input = 0
            while str_input == "" and exit_input == 0:
                clearscreen()
                print_408_409(filter_after, filter_before)
                str_input = input("Input new period ending time (follow the format): ")
                while str_input != "" and str_input [0] == " ":
                    temp = ""
                    for i in range(1, len(str_input)):
                        temp += str_input [i]
        ...
        #showing > filter > list
            ...
            choose = 0
            while int_input == 0 or int_input > choose:
                return_vl = print_410(filter_film, filter_dimension, filter_language, filter_house, filter_after, filter_before)
                choose = return_vl [0]
                int_input = input_int_check("Input number: ", choose)
                clearscreen()
            if int_input == choose:
